...
模型判断为 NG 后，在边缘焊接带内定位空洞：

1. **高亮检测**: `HSV V > 180` 的连通域
2. **局部对比度**: `pixel - median_blur(pixel) > 50` 的连通域
3. 过滤面积过小 / 过大区域
4. 按置信度排序后做 NMS
5. 返回 `DetectionResult` 列表（class_name = `void`）

## 4. 训练结果

使用 `RandomForestClassifier(n_estimators=200, max_depth=12)`：

| 指标 | v1.0（全局特征） | v2.0（全局+边缘特征） |
|------|-----------------|---------------------|
| 测试集准确率 | 79.26% | **80.11%** |
| 5 折交叉验证 | 76.90% ± 1.44% | 78.75% ± 1.45% |
| 全量数据插件评估 | 94.83% | **95.34%** |

按文件夹评估插件表现（v2.0）：

| 文件夹 | 准确率 |
|--------|--------|
| `原始图片/NG` | 89.4% (185/207) |
| `原始图片/Ok` | 95.0% (508/535) |
| `2/原始图片/NG` | 92.1% (233/253) |
| `2/原始图片/Ok` | 98.3% (752/765) |

> 注：95.3% 是在训练数据上的评估结果，实际泛化能力应以 **80.11%** 为准。
